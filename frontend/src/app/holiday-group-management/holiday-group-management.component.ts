...
export class HolidayGroupManagementComponent implements OnInit {
  ...
  getTotalHolidays(): number {
    return this.holidayGroups.reduce(
      (sum, group) => sum + group.holidays.length,
      0,
    );
  }

  getTotalEmployeesCovered(): number {
    return this.holidayGroups.reduce(
      (sum, group) => sum + group.employee_count,
      0,
    );
  }

  getAverageHolidaysPerGroup(): number {
    if (this.holidayGroups.length === 0) return 0;
    return (
      Math.round((this.getTotalHolidays() / this.holidayGroups.length) * 10) /
      10
    );
  }
